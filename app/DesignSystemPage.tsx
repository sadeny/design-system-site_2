...
function DetailModule({ page }: { page: ModulePage }) {
  return <div className="module-page" style={{ "--module-accent": page.accent } as React.CSSProperties}>
    <section className="module-hero"><div className="module-breadcrumb"><span>Design System</span><i>›</i><strong>{page.title}</strong></div><div className="module-hero-grid"><div><p className="section-kicker">{page.eyebrow}</p><h1>{page.title}</h1><p>{page.lead}</p>{page.status && <span className="module-status">{page.status}</span>}</div><div className="module-visual" aria-hidden="true"><span /><span /><span /><i /></div></div></section>
